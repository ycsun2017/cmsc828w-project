...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_sizes, activation, with_clone=False, prior=None, lr=-1):
...
class ContActor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_sizes, activation, action_std, device, with_clone=False, prior=None,
                 lr=-1):
...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_sizes, activation, with_clone=False, prior=None, lr=-1):
        ...
        if not with_clone:
            ...
            # actor
            self.action_layer = clone_mlp(prior.action_layer, [state_dim] + hid + [action_dim], activation, nn.Softmax(dim=-1), lr=lr)
            # critic
            self.value_layer = clone_mlp(prior.value_layer, [state_dim] + hid + [1], activation, lr=lr)
...
class ContActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_sizes, activation, action_std, device, with_clone=False, prior=None,
        ...
        if not with_clone:
            ...
            # action mean range -1 to 1
            self.action_layer = clone_mlp(prior.action_layer, [state_dim] + hid + [action_dim], activation, nn.Tanh(), lr=lr)
            self.action_var = torch.full((action_dim,), action_std * action_std).to(device)
            # critic
            self.value_layer = clone_mlp(prior.value_layer, [state_dim] + hid + [1], activation, lr=lr)
